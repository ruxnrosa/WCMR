...
async function sendInstaDM({ accessToken, recipientId, msgObj }) {
  try {
    ...
    if (!cleanToken) {
      console.error("❌ sendInstaDM: No access token provided");
      return null;
    }
    ...
    if (data?.error) {
      console.error("❌ sendInstaDM error:", data.error);
      return null;
    }

    return { id: data?.message_id || data?.mid || null };
  } catch (err) {
    console.error("❌ sendInstaDM exception:", err);
    return null;
  }
}
  ...
  try {
    ...
    if (origin === "telegram") {
     ...
     WHERE sender_mobile = ? AND uid = ? AND origin = 'instagram_comment' LIMIT 1`,
      ...
      if (!chatInfo) {
        console.error("❌ Chat info not found for instagram_comment reply");
        return null;
      }
      ...
      if (!chatInfo) {
        console.error(
          "❌ Chat info not found for Instagram automation message",
        );
        return null;
      }
...
async function processSetChatLabel({
  ...
  try {
    ...
    console.error("❌ Error in processSetChatLabel:", err);
    return {};
  }
}
...
async function processSendWaTemplate({
  ...
  try {
    ...
    if (!template?.name || !template?.language) {
      console.error("❌ SEND_WA_TEMPLATE: Missing template name or language");
      return { moveToNextNode: node?.data?.moveToNextNode || false };
    }
    ...
    if (!api?.access_token || !api?.business_phone_number_id) {
      console.error("❌ SEND_WA_TEMPLATE: Meta API credentials not found");
      return { moveToNextNode: node?.data?.moveToNextNode || false };
    }
    ...
    console.error("❌ Error in processSendWaTemplate:", err);
    return {};
  }
}
...
async function processSendWaForm({
  ...
  try {
    ...
    if (result?.error) {
      console.error("❌ SEND_WA_FORM Meta error:", result.error);
    }
    ...
    console.error("❌ Error in processSendWaForm:", err);
    return {};
  }
}
