...
router.post("/update_web_config", adminValidator, async (req, res) => {
  try {
    ...
    try {
      ...
      console.error("index.html patch failed:", htmlErr);
    }
...
router.post("/update_insta_config", adminValidator, async (req, res) => {
  try {
    const { insta_app_id, insta_app_secret, insta_callback_url } = req.body;

    if (!insta_app_id || !insta_app_secret || !insta_callback_url) {
      return res.json({ success: false, msg: "Please provide all the fields" });
    }

    await query(
      `UPDATE web_private SET insta_app_id = ?, insta_app_secret = ?, insta_callback_url = ?`,
      [insta_app_id, insta_app_secret, insta_callback_url],
    );
