...
const {
  ...
  processInstaComment,
} = require("../helper/addon/insta/processInatagram.js");
...
router.post("/webhook/:uid", async (req, res) => {
  try {
    ...
    if (!igAccount) {
      // console.log(`[Instagram Webhook] Unknown IG ID: ${igBusinessId}`);
      return;
    }
    ...
    if (commentChanges?.length > 0) {
      console.log("[Instagram] Comment webhook received");
      ...
      if (!user) return console.log("[Instagram Comment] User not found");
      ...
      for (const change of commentChanges) {
        ...
        console.log("[Instagram Comment] Triggering automation for:", {
          uid,
          senderMobile,
